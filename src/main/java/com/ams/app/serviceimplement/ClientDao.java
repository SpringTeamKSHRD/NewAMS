...
public class ClientDao implements ClientService{
	...
	public ArrayList<ArticleDto> getArticleList(ArticleDto art) {
		...
		int begin=art.getLimitRow()*art.getPage()-art.getLimitRow();
	...
	public ArrayList<ArticleDto> searchArticle(ArticleDto art) {
		...
		String sql="SELECT id,title,name,publish_date"
				   + " FROM v_article WHERE Lower("+art.getColumnName()+") LIKE ? LIMIT ? OFFSET ?";
		
		try(
				Connection conn=datasource.getConnection();
				PreparedStatement ps=conn.prepareStatement(sql);
		){
			ps.setString(1, art.getSearch().toLowerCase()+"%");
			ps.setInt(2,art.getLimitRow());
			ps.setInt(3, begin);
			ResultSet rs=ps.executeQuery();
			System.out.println(ps);
			while(rs.next()){
